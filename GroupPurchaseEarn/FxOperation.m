...
{
    NSLog(@"FxOperation:parseData:%ld",data.length);
    ...
    if ([[result lowercaseString] isEqualToString:NetOk]) {
        [self parseSuccess:dict jsonString:jsonString];
    }
    else {
        [self parseFail:[dict objectForKey:NetError]];
    }
...
- (void)parseFail:(NSDictionary *)dict
{
    ...
    [_delegate opFail:[dict objectForKey:NetErrorMessage]];
}
